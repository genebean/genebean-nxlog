...
describe 'nxlog::config::output', :type => :define do
  ...
  context 'On Windows' do
    ...
    describe 'outputting to a local file' do
      ...
      describe 'builds an Output section for the config file which' do
        it { should contain_concat__fragment('output_logserver').with_content(/^<Output logfile>$/) }
        it { should contain_concat__fragment('output_logserver').with_content(/\s\sModule\s+om_file$/) }
        it { should contain_concat__fragment('output_logserver').with_content(/\s\sFile\s+C:\/logfile\.log$/) }
        it { should contain_concat__fragment('output_logserver').with_content(/^<\/Output>$/) }
      end
    end
